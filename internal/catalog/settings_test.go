...
func TestSetFxHouseRoundTrips(t *testing.T) {
	...
	if err := SetFxHouse(db, domain.MEP); err != nil {
...
func TestMarkExportedKeepsTheChosenHouse(t *testing.T) {
	db := openTestStore(t).DB()
	if err := SetFxHouse(db, domain.Official); err != nil {
		t.Fatalf("SetFxHouse() unexpected error: %v", err)
	}

	at := time.Date(2026, time.September, 2, 15, 4, 5, 0, time.UTC)
	if err := MarkExported(db, at); err != nil {
